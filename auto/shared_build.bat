...
@echo.
echo Build %BUILD_APP_OPTIM% shared library
@echo.

call %_EXECNDK% APP_OPTIM=%BUILD_APP_OPTIM% BUILD_SHARED=true PROJECT_PATH=%BUILD_PROJECT_DIR%
if errorlevel 1 goto end

if exist "%libs_dir%" goto okLibs
goto exit
...
for %%c in (%APP_ABI%) do (
    if exist "%libs_dir%\%%c" (
        ...
        copy /y %libs_dir%\%%c\*.so %build_install_dir%\%%c\
    )
)
